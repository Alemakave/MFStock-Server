...
public class DocProps {
    ...
    private void load(InputStream stream) throws Exception {
        ...
        String documentCreatedAsString = workbookDocument.getElementsByTagName("dcterms:created").item(0).getTextContent();
        String documentModifiedAsString = workbookDocument.getElementsByTagName("dcterms:modified").item(0).getTextContent();

        documentCreated = LocalDateTime.parse(documentCreatedAsString.substring(0, documentCreatedAsString.length()-1));
        documentModified = LocalDateTime.parse(documentModifiedAsString.substring(0, documentModifiedAsString.length()-1));
    }
}
