...
public abstract class ExcelStickerGenerator<T extends Sticker> extends StickerGenerator<T> {
    ...
    public ExcelStickerGenerator(Resource stickerTemplateResource) {
        super(StickerFormat.EXCEL);

        this.stickerTemplateResource = stickerTemplateResource;
    }

    @Override
    public byte[] generate(T sticker) throws JsonProcessingException {
        return new byte[0];
    }
    ...
    @Deprecated
    public abstract List<File> generate(File outputFile, T sticker) throws IOException, WriterException;
}
